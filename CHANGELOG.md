...
### Fixed
- **PIM role-policy checks now use the v1.0 API in GCC High (#978)**: the previous `/beta/policies/roleManagementPolicies` call returned `400 MissingProvider` in GCC High. The collector now queries `/v1.0/policies/roleManagementPolicyAssignments` filtered by `roleDefinitionId` with `$expand=policy($expand=rules)`, returning real Pass/Fail. Verified live on GCC High.
- **Power BI connects in GCC High (#943)**: `Connect-PowerBIServiceAccount` is now called with `-Environment USGovHigh` (mapped from `-M365Environment`), and `M365Environment` is threaded into the isolated Power BI child process so the routing actually takes effect across the process boundary. This clears the WAM `IncorrectConfiguration` / redirect-URI error with no Azure portal change. Verified live (12 items collected).
- **Forms and Teams sovereign-cloud gaps emit `Skipped` instead of disappearing (#941, #940)**: the Forms settings endpoint and the two beta Teams endpoints (`teamsClientConfiguration`, `teamsMeetingPolicy`) have no v1.0 equivalent and return `400` in GCC High. Their dependent checks (1 Forms + 6 Teams external/client + 9 Teams meeting-policy) now emit `Skipped` with a clear "not available in this cloud" message and appear in the report's not-assessed group.
- **Teams license gate recognizes sovereign service plans (#940)**: the gate previously checked the commercial `TEAMS1` plan plus a GUID that is actually `WHITEBOARD_PLAN3`, so Teams checks were silently skipped on every government tenant. It now matches `TEAMS_GOV`, `TEAMS_AR_GCCHIGH`, and `TEAMS_AR_DOD` (verified against Microsoft's licensing reference).
- **Compliance licensing overlay uses real service-plan names (#980)**: `COMPLIANCE-DLP-002`, `COMPLIANCE-LABELS-002`, and `COMPLIANCE-COMMS-001` were gated on a SKU id, a nonexistent plan name, and a retired plan name, so they false-skipped (DLP-002 on every cloud). The overlay now also accepts `COMMUNICATIONS_DLP`, `RMS_S_PREMIUM2`/`MIP_S_CLP2`, and `MICROSOFT_COMMUNICATION_COMPLIANCE`.
- **Hardcoded `graph.microsoft.com` URIs replaced with relative paths (#945)** so requests route to `graph.microsoft.us` in sovereign clouds.
- **Secure Score confirmed working in GCC High (#939)** on v2.12.0+; no graceful-degradation change was needed.
- **Posture score card no longer renders `NaN` (#967)**: `Posture()` guards `score` and the peer-average with `Number.isFinite` and shows an "unavailable" message when Secure Score data is absent.
- **Removed the no-op License-Adjusted scoring tab (#966)**: it was dead code (its `NotLicensed` filter never differed from Security Risk) and always showed an identical number. Redesign tracked in #976.
- **"Critical exposure" report section revived (#968)**: the curated critical-exposure analysis (privileged account exposure, Conditional Access exclusions, dangerous Graph permissions, audit-trail gaps) had been silently dead since the registry was re-prefixed - its React component was never mounted and its filter matched a `'Stryker Readiness'` domain the pipeline never produces. It now renders from a `criticalExposure` flag on each finding, mounts after the Overview, and gains a Sidebar nav entry. The "Stryker" codename is dropped in favor of plain "Critical exposure"; the flag accepts both the legacy and renamed collector ids so the upstream CheckID rename syncs with no report change.
- **HIPAA framework coverage matched nothing (#918 review)**: `hipaa.json` criteria keys carried a section sign (`§164.306`) that never matched the registry's bare controlIds (`164.306`), so every HIPAA finding fell through as uncovered. Keys are now bare CFR numbers and the file is fully ASCII.

### Documentation
- **GCC High setup guide (#944)**: new `docs/user/GCC-HIGH-SETUP.md` covering app registration at `portal.azure.us`, consent, Power BI routing, and the known sovereign gaps; the sovereign-cloud support matrix is refreshed to the verified state.
- **README trimmed to a landing page (#923)**: 479 to 126 lines; the section catalogue, full parameter reference, connection profiles, module management, and standalone-script usage moved to `docs/user/RUN.md`, and the stale "What's New" block is replaced with a CHANGELOG link.
- **Architecture diagrams refreshed (#918)**: the four Mermaid diagrams in `docs/diagrams/` are corrected to the current code (check count 292, 15 frameworks, `SPO-*` prefix, dead ScubaGear references pruned) with the baseline/drift and local-extension-preservation flows added.
